...
public class StarEnigma {
    public static void main(String[] args) {
        ...
        String dataRegex = "^[^@\\-!:>]*@(?<planet>[A-Za-z]+)[^@\\-!:>]*:(?<population>[\\d]+)[^@\\-!:>]*!(?<attackType>[AD])![^@\\-!:>]*->(?<soldierCount>\\d+)[^@\\-!:>]*$";
        Pattern messagePattern = Pattern.compile(dataRegex);
        ...
        for (int i = 0; i < n; i++) {
            ...
            Matcher dataMatcher = messagePattern.matcher(sb.toString());

            if (dataMatcher.find()) {
                String planet = dataMatcher.group("planet");
                String attackType = dataMatcher.group("attackType");
